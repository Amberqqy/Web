$(function(){
    ...
    "health_news":[
        ...
        {
            "favor_num":2,
            "img":"https://media2.chunyuyisheng.com/@/media/images/2017/11/21/387b/74b006dc3101_w180_h150_.jpg?imageView2/1/w/120/h/116",
            "title":"辣椒到底致癌还是抗癌？最靠谱的答案是…… ",
            "date_str":"1小时前",
            "image":"https://media2.chunyuyisheng.com/@/media/images/2017/11/21/387b/74b006dc3101_w180_h150_.jpg?imageView2/1/w/120/h/116",
            "news_id":113985,
            "channel_id":18,
            "view_times":1518,
            "type":"news",
            "id":113985,
            "digest":"有辣椒致癌的文献，不过可靠性较低。关于辣椒抗癌的研究结果很多，但也只是说明“辣椒素可能具有抗癌能力”。 综合这些研究结果，我们可以认为：经常吃辣，很可能是有益健康的——至少，不必担心危害。",
            "channel_name":"辟谣"
        },
        {
            "description":"",
    ...
    $(".illness-text").html(illnessStr);
    var healthStr ="";
    for(var j=0; j<res.health_news.length; j++){
        healthStr +='<div class="text-box"><a class="health-text" href="javascript:void(0)"><img src="'+res.health_news[j].image+'" alt=""><span>'+res.health_news[j].title+'</span><p><span>'+res.health_news[j].channel_name+'</span> &nbsp <span>'+res.health_news[j].date_str+'</span></p></a></div>'
    }
    $(".health-box").html(healthStr);
    var answerStr ="";
    for(var k=0; k<res.hot_problems.length; k++){
        answerStr +='<div class="text-box"><a class="answer-text" href="javascript:void(0)"><h6><span class="text-tag">问</span>'+res.hot_problems[k].title+'</h6><p>'+res.hot_problems[k].ask+'</p></a></div>'
    }
    $(".answer-box").html(answerStr);
    
    var swiperStr ="";
    for(var m=0; m<res.banner_list.length; m++){
        swiperStr  +='<a href="javascript:void(0)" class="swiper-slide main-slide"><img src="'+res.banner_list[m].image+'" alt=""></a>'
    }
	...
	// 		var str ="";
 //            for(var i=0; i<res.popular_diseases.length; i++){
 //            	str +='<a href="javascript:void(0)">'+res.popular_diseases[i].name+'</a>'
 //            }
 //            $(".illness-text").html(str);
	// 	},
	// 	error:function(res){
